...
def prepare_batch(batch, model):
    ...
    ast_node_rep = batch['ast_node_rep']
    ast_len = batch['ast_len']
    
    if args.cuda:
        code_len = batch['code_len'].cuda(non_blocking=True)
        ast_len = batch['ast_len'].cuda(non_blocking=True)
        ...
        if ast_node_rep is not None:
            ast_node_rep = ast_node_rep.cuda(non_blocking=True)
    ...
    batch_inputs['ast_node_rep'] = ast_node_rep
    batch_inputs['ast_len'] = ast_len
    return batch_inputs
