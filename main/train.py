...
def add_train_args(parser):
    ...
    files.add_argument('--train_ast', nargs='+', type=str,
                       help='Preprocessed train ast file')
    files.add_argument('--dev_ast', nargs='+', type=str, required=True,
                       help='Preprocessed dev ast file')
    ...
    preprocess.add_argument('--ast_vocab_size', type=int, default=20000,
                            help='Maximum allowed length for ast dictionary')
...
def init_from_scratch(args, train_exs, dev_exs):
    ...
    ast_dict = util.build_word_and_char_dict(args,
                                             ...
                                             no_special_token=True)
    logger.info('Num words in source = %d and target = %d and ast = %d' % (len(src_dict), len(tgt_dict), len(ast_dict)))

    # Initialize model
    model = Code2NaturalLanguage(config.get_model_args(args), src_dict, tgt_dict, ast_dict)
...
def main(args):
    ...
    if not args.only_test:
        ...
        for train_src, train_src_tag, train_tgt, train_ast, dataset_name in \
                zip(args.train_src_files, args.train_src_tag_files,
                    args.train_tgt_files, args.train_ast_files, args.dataset_name):
            train_files = dict()
            train_files['src'] = train_src
            train_files['src_tag'] = train_src_tag
            train_files['tgt'] = train_tgt
            train_files['ast'] = train_ast  # ast序列文件位置
    ...
    dev_exs = []
    for dev_src, dev_src_tag, dev_tgt, dev_ast, dataset_name in \
            zip(args.dev_src_files, args.dev_src_tag_files,
                args.dev_tgt_files, args.dev_ast_files, args.dataset_name):
        dev_files = dict()
