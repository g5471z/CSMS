...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.ast_attn = MultiHeadedAttention(
            heads, d_model, d_k, d_v, dropout=dropout,
    ...
    def forward(self,
        ...
        mid, attn, coverage = self.ast_attn(ast_memory,
                                            ...
                                            step=step,
                                            coverage=coverage)
        mid_norm_1 = self.layer_norm_2(self.drop(mid) + mid_norm)

        output = self.feed_forward(mid_norm_1)
...
class TransformerDecoder(DecoderBase):
    ...
    def init_state(self, src_len, max_len, ast_len, max_ast_len):
        """Initialize decoder state."""
        state = dict()
        state["src_len"] = src_len  # [B]
        state["src_max_len"] = max_len  # an integer
        state["ast_len"] = ast_len  # [B]
        state["ast_max_len"] = max_ast_len  # an integer
        state["cache"] = None
        return state
    ...
    def forward(self,
        ...
        ast_pad_mask = ~sequence_mask(state["ast_len"],
                                      max_len=state["ast_max_len"]).unsqueeze(1)
