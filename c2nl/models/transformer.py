...
class Embedder(nn.Module):
    def __init__(self, args):
        ...
        self.use_ast_node = args.use_asts  #111
        ...
        if self.use_ast_node:
            self.ast_node_embeddings = Embeddings(args.emsize,
                                                  args.ast_vocab_size,
                                                  constants.PAD)
        ...
        self.ast_pos_emb = args.ast_pos_emb   #1111
        ...
        if self.ast_pos_emb:
            self.ast_pos_embeddings = nn.Embedding(args.max_ast_len,
                                                   self.enc_input_size)
    ...
    def forward(self,
        ...
        if mode == 'encoder':
            word_rep = None
            if self.use_src_word:
                word_rep = self.src_word_embeddings(sequence.unsqueeze(2))  # B x P x d    batch x len x embedding_size
            if self.use_src_char:
            ...
            if self.use_ast_node:
                ...
                ast_pos_enc = torch.arange(start=0,
                                       end=ast_rep.size(1)).type(torch.LongTensor)
                ast_pos_enc = ast_pos_enc.expand(*ast_rep.size()[:-1])
                
                if ast_rep.is_cuda:
                    ast_pos_enc = ast_pos_enc.cuda()
                ast_pos_rep = self.ast_pos_embeddings(ast_pos_enc)
                ast_rep = ast_rep + ast_pos_rep
                ast_rep = self.dropout(ast_rep)
            ...
            if self.tgt_pos_emb:
                if step is None:
                    ...
                    #print("pos_enc1:", pos_enc)

                pos_enc = pos_enc.expand(*word_rep.size()[:-1])
                #print("*word_rep.size:", *word_rep.size())
                if word_rep.is_cuda:
                    pos_enc = pos_enc.cuda()
                pos_rep = self.tgt_pos_embeddings(pos_enc)
               # print("pos_rep:", pos_rep.size())
                word_rep = word_rep + pos_rep
        ...
        return word_rep, ast_rep

class Convolutional(nn.Module):
    def __init__(self, args):
        super(Convolutional, self).__init__()
        self.cnn = nn.Conv2d(in_channels=1, out_channels=1, kernel_size=(3, 1), padding=(1, 0), stride=(1, 1))
        self.mxp = nn.MaxPool2d(kernel_size=(2, 1), stride=(2, 1))
        self.embedding_dim = args.emsize
        self.max_len = args.max_src_len

    def forward(self, code_rep, src_len):

#         len = code_rep.size(1)

#         if(len > (self.max_len // 2)):

#             code_rep = code_rep.view(-1, len, self.embedding_dim, 1)
#             code_rep = code_rep.permute(0, 3, 1, 2)
#             #         conv1_w = nn.Parameter(torch.Tensor(3, 1, 1, 1))
#             #         torch.nn.init.xavier_uniform_(conv1_w)

#             input_emb1 = self.cnn(code_rep)
#             input_emb1 = self.mxp(input_emb1)

#             code_rep = input_emb1.permute(0, 2, 3, 1).contiguous().view(-1, len // 2, self.embedding_dim)

#             src_len = src_len // 2

        return code_rep, src_len
...
class Encoder(nn.Module):
    ...
    def forward(self,
        ...
        if input1 is None:
            ...
            layer_outputs, _ = self.transformer(input, input_len, input1)
        if self.use_all_enc_layers:
...
#             ast_memory = ast_layer_outputs[-1]
...
class Decoder(nn.Module):
    ...
    def init_decoder(self,
        ...
        if self.split_decoder:
            state_c = self.transformer_c.init_state(src_lens, max_src_len, ast_lens, max_ast_len)
            state_d = self.transformer_d.init_state(src_lens, max_src_len, ast_lens, max_ast_len)
            return state_c, state_d
        else:
            return self.transformer.init_state(src_lens, max_src_len, ast_lens, max_ast_len)
    ...
    def forward(self,
        ...
        max_ast_len = ast_memory[0].shape[1] \
            if isinstance(ast_memory, list) else ast_memory.shape[1]
        state = self.init_decoder(memory_len, max_mem_len, ast_len, max_ast_len)
        return self.decode(tgt_pad_mask, tgt_emb, memory_bank, ast_memory, state)
...
class Transformer(nn.Module):
    ...
    def __init__(self, args, tgt_dict):
        ...
        self.convolution = Convolutional(args)
        
        self.encoder = Encoder(args, self.embedder.enc_input_size)
        
#         self.ast_encoder = Encoder(args, self.embedder.enc_input_size)
#         self.ctoa_encoder = Encoder(args, self.embedder.enc_input_size)
#         self.atoc_encoder = Encoder(args, self.embedder.enc_input_size)
    ...
    def _run_forward_ml(self,
        ...
        code_rep, ast_rep = self.embedder(code_word_rep,
                                          code_char_rep,
                                          ast_node_rep,
                                          code_type_rep,
                                          mode='encoder')
        
#         print("code_word_rep_size000:",code_word_rep[0].size())
#         print("code_word_rep_size666:",code_word_rep[6].size())# [16,429]
#         print("code_rep.size:",code_rep.size())     # [16,429,512]
        
#         len = code_rep.size(1)
        
#         code_rep = code_rep.view(-1, len, self.embedder.enc_input_size, 1)
#         code_rep = code_rep.permute(0, 3, 1, 2)
# #         conv1_w = nn.Parameter(torch.Tensor(3, 1, 1, 1))
# #         torch.nn.init.xavier_uniform_(conv1_w)
...
#         temp_code = memory_bank
#         temp_ast = ast_memory
# #         print("code:", temp_code.shape)
# #         print("ast:", temp_ast.shape)
# #         print("memory_bank:", memory_bank.shape)
#         memory_bank, layer_wise_outputs = self.ctoa_encoder(temp_ast, ast_len, temp_code)
#         ast_memory, ast_layer_wise_outputs = self.atoc_encoder(temp_code, code_len, temp_ast)

        # embed and encode the target sequence
        summ_emb, _ = self.embedder(summ_word_rep,
                                 summ_char_rep,
                                 mode='decoder')
        ...
        ast_enc_outputs = ast_layer_wise_outputs if self.layer_wise_attn else ast_memory
    ...
    def __generate_sequence(self,
        ...
        max_ast_len = params['ast_memory'][0].shape[1] \
            if isinstance(params['ast_memory'], list) else params['ast_memory'].shape[1]
        dec_states = self.decoder.init_decoder(params['src_len'], max_mem_len, params['ast_len'], max_ast_len)

        attns = {"coverage": None}
        enc_outputs = params['layer_wise_outputs'] if self.layer_wise_attn \
            else params['memory_bank']
        ast_enc_outputs = params['ast_layer_wise_outputs'] if self.layer_wise_attn \
            else params['ast_memory']

        # +1 for <EOS> token
        for idx in range(params['max_len'] + 1):
            tgt, _ = self.embedder(tgt_words,
                                tgt_chars,
                                mode='decoder',
                                step=idx)
            #print("tgt_word:", tgt_words)
            ...
            decoder_outputs = layer_wise_dec_out[-1]
            #print("decoder_outputs1:", decoder_outputs.size())
            #print("decoder_outputs2:", decoder_outputs.squeeze(1).size())
    ...
    def decode(self,
        ...
        word_rep, ast_rep = self.embedder(code_word_rep,
                                          code_char_rep,
                                          ast_node_rep,
                                          code_type_rep,
                                          mode='encoder')
#         word_rep, code_len = self.convolution(word_rep, code_len)
...
#         ast_memory, ast_layer_wise_outputs = self.atoc_encoder(temp_code, code_len, temp_ast)
        ...
        params['ast_memory'] = ast_memory
        params['ast_layer_wise_outputs'] = ast_layer_wise_outputs
        params['ast_len'] = ast_len
