...
def vectorize(ex, model):
    ...
    ast_dict = model.ast_dict

    code, summary, ast = ex['code'], ex['summary'], ex['ast']
    vectorized_ex = dict()
    ...
    vectorized_ex['use_asts'] = False
    if model.args.use_asts:
        vectorized_ex['ast_node_rep'] = torch.LongTensor(ast.vectorize(word_dict=ast_dict))
        vectorized_ex['use_asts'] = True

    vectorized_ex['ast'] = ast.text
    vectorized_ex['ast_tokens'] = ast.tokens
...
def batchify(batch):
    ...
    # --------- Prepare AST tensors ---------
    use_asts = batch[0]['use_asts']
    ast_words = [ex['ast_node_rep'] for ex in batch]
    max_ast_len = max([d.size(0) for d in ast_words])
    ast_len_rep = torch.zeros(batch_size, dtype=torch.long)
    ast_node_rep = torch.zeros(batch_size, max_ast_len, dtype=torch.long) \
        if use_asts else None

    # ast_maps = []
    for i in range(batch_size):
        ast_len_rep[i] = ast_words[i].size(0)
        if use_src_word:
            ast_node_rep[i, :ast_words[i].size(0)].copy_(ast_words[i])
        # ast_tokens = batch[i]['ast_tokens']
        # ast_map = torch.LongTensor([vocab[w] for w in ast_tokens])
        # ast_maps.append(ast_map)
    ...
    return {
        ...
        'ast_len': ast_len_rep,
        'ast_text': [ex['ast'] for ex in batch],
        'ast_tokens': [ex['ast_tokens'] for ex in batch],
    }
