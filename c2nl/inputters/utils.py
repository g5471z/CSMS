...
def load_data(args, filenames, max_examples=-1, dataset_name='java',
    ...
    # 读取ast序列
    with open(filenames['ast'],encoding='utf-8') as f:
        asts = [line.strip() for line in
                tqdm(f, total=count_file_lines(filenames['ast']))]

    assert len(sources) == len(source_tags) == len(targets) == len(asts)

    examples = []
    for src, src_tag, tgt, ast in tqdm(zip(sources, source_tags, targets, asts),
                                       total=len(sources)):
