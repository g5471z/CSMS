...
class Translator(object):
    ...
    def translate_batch(self, batch_inputs):
        ...
        ast_node_rep = batch_inputs['ast_node_rep']
        ast_len = batch_inputs['ast_len']
...
#                             code_type_rep,
#                             mode='encoder')
        code_rep, ast_rep = embedder(code_word_rep,
                                     code_char_rep,
                                     ast_node_rep,
                                     code_type_rep,
                                     mode='encoder')
        ...
        if model_name == 'Transformer':
            ...
            ast_memory, ast_layer_wise_outputs = encoder(ast_rep, ast_len)
            src_lens = code_len.repeat(beam_size)
            ast_lens = ast_len.repeat(beam_size)
            dec_states = decoder.init_decoder(src_lens, memory_bank.shape[1], ast_lens, ast_memory.shape[1])
        else:
        ...
        if model_name == 'Transformer' and model.layer_wise_attn:
            ...
            ast_memory = [rvar(lwo.data) for lwo in ast_layer_wise_outputs]
        else:
            memory_bank = rvar(memory_bank.data)
            ast_memory = rvar(ast_memory.data)
        ...
        for i in range(self.max_length + 1):
            ...
            if model_name == 'Transformer':
                tgt, _ = embedder(inp, inp_chars, mode='decoder', step=i)
                # Run one step.
                ...
                # rnn-based model
                tgt, _ = embedder(inp, inp_chars, mode='decoder')
